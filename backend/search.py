...
api_search_url = config.api_base_url.copy()  # https://music.163.com/api/search/get/web
# api_search_url["C"] += "/search?keywords={}&type={}&limit={}&offset={}"
api_search_url[
    "C"
] += "?csrf_token=hlpretag=&hlposttag=&s={}&type={}&limit={}&total=true&offset={}"
api_search_url["B"] = (
    "https://api.bilibili.com/x/web-interface/search/type?keyword={}&search_type={}&page={}"
)
...
def cloud_search(keyword, Type, limit, offset):
    ...
    if Type == typet["music"] or Type == typet["lrc"]:
        ...
        for ind, i in enumerate(dic):  # ind索引 i为对象
            x = {}
            x["type"] = "music"
            x["mid"] = "C" + str(i["id"])
            x["name"] = i["name"]
